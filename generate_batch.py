#!/usr/bin/env python3
"""Vyberie N nepouzitych tem z topics_bank.json, vyrenderuje cez make_video.py, oznaci ako pouzite."""
import json
import os
import re
import subprocess
...
def main():
    ...
    if not remaining:
        print("Vsetky temy pouzite. Pridaj nove do topics_bank.json (alebo nechaj generate_topics)."); return
    batch = remaining[:n]
    ...
    for i, spec in enumerate(batch, 1):
        ...
        if r.returncode == 0:
            made.append(title); used.append(title)
            json.dump(used, open(STATE, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
        else:
            print(f"[CHYBA] render zlyhal: {title}")
    print(f"\n========== HOTOVO: {len(made)} videi ==========")
