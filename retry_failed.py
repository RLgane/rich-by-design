...
def main():
    ...
    if not token:
        print("retry: chyba buffer_token."); return
    ...
    try:
        ...
        edges = gql('query { posts(input:{organizationId:"%s", filter:{status:[error]}}){ edges { node { %s } } } }'
                    % (org, node))["posts"]["edges"]
    except Exception as e:
        print("retry: Buffer chyba:", str(e)[:160]); return

    if not edges:
        print("retry: ziadne chybne posty."); return
    print("retry: %d chybnych postov..." % len(edges))
    ok = 0
    for e in edges:
        ...
        try:
            ...
            pass
        time.sleep(1)
    print("retry: znova zaradenych %d." % ok)
