#!/usr/bin/env python3
"""
Faceless factory - automaticka tvorba vertikalnych (9:16) faceless videi.
Pipeline: edge-tts hlas + casovanie slov -> Pexels B-roll -> FFmpeg (titulky, pohyb, hudba) -> MP4 + .txt
Pouzitie: python make_video.py scripts/sample.json [--open] [--no-captions]
"""
import argparse
import asyncio
import json
...
def trim_trailing_silence(ff, src, dst, gap=0.12):
    """Odreze dlhe ticho na konci segmentu, necha jednotnu pauzu (gap s). Trailing-only
    (cez areverse) -> casovanie slov pre titulky ostava platne."""
...
def build_ass(all_words, cfg, path):
    ...
    style = cfg.get("caption_style", "box")
    box_hex = cfg.get("caption_box_hex", "FF901E")
    text_hex = cfg.get("caption_text_hex", "FFFFFF")
    hl = cfg.get("caption_highlight_hex", "00F2FF")
    pop = cfg.get("caption_pop_scale", 116)

    if style == "box":
        border_style, outline, shadow = 3, 16, 5
        outline_col = f"&H00{box_hex}"
        back_col = "&HA0000000"
    else:
...
def add_sfx(ff, video, cut_times, tmp):
    """Jemny 'whoosh' na kazdy strih. Pri chybe vrati povodne video."""
    if not cut_times:
        return video
    try:
...
def main():
    ...
    print(f"  Skladam {len(seg_files)} segmentov (~{cursor:.1f}s)...")
